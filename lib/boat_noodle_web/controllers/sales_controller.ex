defmodule BoatNoodleWeb.SalesController do
  ...
  defp item_sales_report_csv_content(conn, params) do
    ...
    item_sales_outlet
    |> Stream.map(fn x ->
      combo_item_report_csv_content(x, conn, params,staffs, subcat_data, combo_data, combo_data_price)
    end)
  ...
  defp discount_item_report_csv_content(conn, params) do
    ...
    data =
      for item <- discount_item_report_csv_content do
        discount_amount =Decimal.to_float(item.grand_total)-(Decimal.to_float(item.sub_total) + Decimal.to_float(item.service_charge)+ Decimal.to_float(item.gst_charge)+ Decimal.to_float(item.rounding))
          ...
          from(
            ...
            where:
              ...
              sd.brand_id == ^brand.id and
                di.brand_id == ^brand.id and 
                s.brand_id == ^brand.id and
                st.brand_id == ^brand.id and
                 d.brand_id == ^brand.id and  
                b.brand_id == ^brand.id and 
                br.id == ^brand.id and
                sp.brand_id == ^brand.id and
                st.brand_id == ^brand.id and 
                s.branchid == ^branch_id and
                s.salesdate >= ^start_date and 
                s.salesdate <= ^end_date,
    ...
    data =
      for item <- discount_item_detail_report_csv do
       ...
       after_disc=Decimal.to_float(item.afterdisc)
         ...
         discount_amount =if item.disctype == "VOUCHER" do
