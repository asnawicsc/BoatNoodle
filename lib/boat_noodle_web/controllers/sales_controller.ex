defmodule BoatNoodleWeb.SalesController do
  ...
  defp item_sales_report_csv_content(conn, params) do
      ...
      if branch_id != "0" do
        ...
        Repo.all(
          from(
            ...
            left_join: s in BoatNoodle.BN.Sales,
            on: s.salesid == sd.salesid,
            group_by: [sd.cat_name, sd.itemname],
            where:
              s.is_void == 0 and sd.is_void == 0 and s.branchid == ^branch_id and
                s.salesdate >= ^start_date and s.salesdate <= ^end_date and
                sd.brand_id == ^brand.id and s.brand_id == ^brand.id,
            select: %{
              itemname: sd.itemname,
              itemcatname: sd.cat_name,
              qty: sum(sd.qty),
              nett_price: sum(sd.afterdisc),
              gross_price: sum(sd.order_price)
            }
        ...
        Repo.all(
          from(
            ...
            group_by: [sd.cat_name, sd.itemname],
            where:
              s.is_void == 0 and sd.is_void == 0 and s.salesdate >= ^start_date and
                s.salesdate <= ^end_date and sd.brand_id == ^brand.id and s.brand_id == ^brand.id,
            select: %{
              itemname: sd.itemname,
              itemcatname: sd.cat_name,
              qty: sum(sd.qty),
    ...
    csv_content = ['Item Name', 'Item Category', 'Quantity', 'Net Sales', 'Gross Sales']
  ...
  def combo_item_report_csv(conn, params) do
    ...
    item_sales_outlet =
      if branch_id != "0" do
        ...
        Repo.all(
          from(
            sd in BoatNoodle.BN.SalesDetailCombo,
            left_join: s in BoatNoodle.BN.Sales,
            on: s.salesid == sd.sales_id,
            left_join: b in BoatNoodle.BN.Branch,
            on: b.branchid == s.branchid,
            group_by: [b.branchname, sd.itemname, sd.combo_name],
            where:
              s.salesdate >= ^start_date and s.branchid == ^branch_id and s.salesdate <= ^end_date and
                s.brand_id == ^brand.id and b.brand_id == ^brand.id and s.is_void == ^0 and
                sd.is_combo_header != 1 and sd.is_void == ^0,
            select: %{
              branchname: b.branchname,
              itemname: sd.itemname,
              gross_qty: sum(sd.qty),
        ...
        Repo.all(
          from(
            ...
            where:
              s.salesdate >= ^start_date and s.salesdate <= ^end_date and s.brand_id == ^brand.id and
                b.brand_id == ^brand.id and s.is_void == ^0 and sd.is_combo_header != 1 and
                sd.is_void == ^0,
            select: %{
              branchname: b.branchname,
              itemname: sd.itemname,
  ...
  defp combo_item_report_csv_content(
    ...
    nett_qty = Decimal.to_integer(item.gross_qty)
    -Decimal.to_integer(item.foc_qty)
  ...
  def item_sales_outlet_csv(conn, params) do
    ...
    {item_sales_outlet} =
      if branch_id != "0" do
        ...
        a =
          Repo.all(
            from(
              ...
              left_join: b in BoatNoodle.BN.Branch,
              on: b.branchid == s.branchid,
              group_by: [s.salesdate, b.branchname, sd.cat_name, sd.itemname],
              where:
                s.is_void == 0 and sd.is_void == 0 and s.branchid == ^branch_id and
                  s.salesdate >= ^start_date and s.salesdate <= ^end_date and
                  sd.brand_id == ^brand.id and s.brand_id == ^brand.id and b.brand_id == ^brand.id,
              select: %{
                salesdate: s.salesdate,
                branchname: b.branchname,
                itemname: sd.itemname,
        ...
        b =
          Repo.all(
            from(
              ...
              left_join: b in BoatNoodle.BN.Branch,
              on: b.branchid == s.branchid,
              group_by: [s.salesdate, b.branchname, sd.itemname, sd.combo_name],
              where:
                s.salesdate >= ^start_date and s.salesdate <= ^end_date and
                  s.brand_id == ^brand.id and b.brand_id == ^brand.id and sd.brand_id == ^brand.id and
                  s.is_void == ^0 and sd.is_combo_header != 1 and sd.is_void == ^0 and
                  s.branchid == ^branch_id,
              select: %{
                salesdate: s.salesdate,
                branchname: b.branchname,
                itemname: sd.itemname,
                itemcatname: sd.catname,
                qty: sum(sd.qty),
                foc_qty: sum(sd.foc),
                gross_sales: sum(sd.order_price),
                nett_sales: sum(sd.afterdisc),
        ...
        b =
          for item <- b do
            %{
              ...
              qty: item.qty,
              foc_qty: Decimal.to_integer(item.foc_qty),
        ...
        {c |> Enum.sort_by(fn x -> x.salesdate end)}
      else
        # Repo.all(
        #   from(
        #     sd in BoatNoodle.BN.SalesMaster,
        ...
        a =
          Repo.all(
            from(
              ...
              where:
                s.is_void == 0 and sd.is_void == 0 and s.salesdate >= ^start_date and
                  s.salesdate <= ^end_date and sd.brand_id == ^brand.id and
                  s.brand_id == ^brand.id and b.brand_id == ^brand.id,
              select: %{
                salesdate: s.salesdate,
                branchname: b.branchname,
                itemname: sd.itemname,
          ...
          Repo.all(
            from(
              ...
              where:
                s.salesdate >= ^start_date and s.salesdate <= ^end_date and
                  s.brand_id == ^brand.id and b.brand_id == ^brand.id and sd.brand_id == ^brand.id and
                  s.is_void == ^0 and sd.is_combo_header != 1 and sd.is_void == ^0,
              select: %{
                salesdate: s.salesdate,
                branchname: b.branchname,
                itemname: sd.itemname,
  ...
  defp item_sales_outlet_csv_content(
    ...
    # foc_qty=if item.foc_qty == do

    # end

    nett_qty = Decimal.to_integer(item.qty) - item.foc_qty
  ...
  defp item_sales_outlet_csv_content_v2(item, conn, params, brand) do
    item_qty =
      if item.qty == nil do
        0
      else
