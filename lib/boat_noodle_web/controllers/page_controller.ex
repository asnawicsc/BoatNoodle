defmodule BoatNoodleWeb.PageController do
  ...
  def index(conn, _params) do
    ...
    total =
      Repo.all(from(s in BoatNoodle.BN.Sales, where: s.branchid == "1" and s.salesdate == ^date))
    ...
    total_transaction =
      Repo.all(
        from(
          ...
          where: s.branchid == "1" and s.salesdate == ^date,
          select: %{gst_charge: sp.gst_charge, grand_total: sp.grand_total, salesid: s.salesid}
        )
      )
    ...
    sales_graph =
      Repo.all(
        from(
          ...
          on: s.salesid == sp.salesid,
          where: s.branchid == "1" and s.salesdate == ^date,
          select: %{sales: sum(sp.grand_total)}
        )
      )
      |> hd()
      |> Enum.map(fn x -> elem(x, 1) end)
    ...
    discount =
      Repo.all(
        from(
          ...
          on: s.salesid == sp.salesid,
          where: s.branchid == "1" and s.salesdate == ^date,
          select: %{discount: sum(sp.after_disc)}
        )
      )
      |> hd()
      |> Enum.map(fn x -> elem(x, 1) end)
    ...
    sub_total =
      Repo.all(
        from(
          ...
          on: s.salesid == sp.salesid,
          where: s.branchid == "1" and s.salesdate == ^date,
          select: %{sub_total: sum(sp.sub_total)}
        )
      )
      |> hd()
      |> Enum.map(fn x -> elem(x, 1) end)
    ...
    transaction_graph =
      Repo.all(
        from(
          ...
          on: s.salesid == sp.salesid,
          where: s.branchid == "1" and s.salesdate == ^date,
          select: %{trasaction: count(sp.salespay_id)}
        )
      )
      |> hd()
      |> Enum.map(fn x -> elem(x, 1) end)
    ...
    top_10_sales_item =
      Repo.all(
        from(
          ...
          group_by: [sd.itemid],
          select: %{qty: count(sd.itemid), itemname: i.itemname},
          order_by: [desc: count(sd.itemid)],
          limit: 10
        )
      )
    ...
    item_sales_by_category =
      Repo.all(
        from(
          ...
          on: s.salesid == sd.salesid,
          left_join: i in BoatNoodle.BN.ItemSubcat,
          on: i.subcatid == sd.itemid,
          left_join: is in BoatNoodle.BN.ItemCat,
          on: is.itemcatid == i.itemcatid,
          where: s.branchid == "1" and s.salesdate == ^date,
    ...
    total_sales =
      Repo.all(
        from(
          sp in BoatNoodle.BN.SalesPayment,
          left_join: s in BoatNoodle.BN.Sales,
          where: s.salesid == sp.salesid and s.branchid == "1",
    ...
    total_visits =
      Repo.all(
        from(
          ...
          left_join: s in BoatNoodle.BN.Sales,
          where: s.salesid == sp.salesid and s.branchid == "1",
          select: %{datetime: s.salesdatetime, pax: s.pax}
        )
      )
    ...
    year_map =
      total_sales
      |> Enum.map(fn x ->
        %{year: x.datetime.year, month: x.datetime.month, grand_total: x.grand_total}
      end)
    ...
    year_map1 =
      total_taxes
      |> Enum.map(fn x ->
        %{year: x.datetime.year, month: x.datetime.month, gst_charge: x.gst_charge}
      end)
      |> Enum.group_by(fn x -> x.year end)
    ...
    totals =
      for month <- months do
        final =
          month_map[month] |> Enum.map(fn x -> Decimal.to_float(x.grand_total) end) |> Enum.sum()
    ...
    totals1 =
      for month <- months1 do
        final =
          month_map1[month] |> Enum.map(fn x -> Decimal.to_float(x.gst_charge) end) |> Enum.sum()
    ...
    render(
      ...
      outlet_sales_information: outlet_sales_information
    )
  end

  def sales_graph_by_year(conn, %{"year" => year}) do
    date = "2018-01-07"
