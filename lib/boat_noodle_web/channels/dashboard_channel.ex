...
# 
      ...
      grp_daily=for item <- grp do

        date=item|>elem(0)
        data=item|>elem(1)
        total_after_disc=Enum.map(data,fn x -> Decimal.to_float(x.after_disc) end)|>Enum.sum
        total_sub_total=Enum.map(data,fn x -> Decimal.to_float(x.sub_total) end)|>Enum.sum
        total_sales=Enum.map(data,fn x -> Decimal.to_float(x.grand_total) end)|>Enum.sum|>Float.round(2)
        total_taxes=Enum.map(data,fn x -> Decimal.to_float(x.gst) end)|>Enum.sum|>Float.round(2)
        total_discount=total_after_disc - total_sub_total|>Float.round(2)
        total_service_charge=Enum.map(data,fn x -> Decimal.to_float(x.service_charge) end)|>Enum.sum|>Float.round(2)
        total_transaction=Enum.map(data,fn x -> x.transaction end)|>Enum.sum
 

        %{date: date, total_sales: total_sales, total_taxes: total_taxes,
         total_discount: total_discount, total_service_charge: total_service_charge,total_transaction: total_transaction}
     ...
     for data <- group_data do
          ...
          abc=for item <- data_all do
            ...
            if name == data do
               after_disc=Enum.map(item,fn x -> Decimal.to_float(x.after_disc) end)|>Enum.sum
                sub_total=Enum.map(item,fn x -> Decimal.to_float(x.sub_total) end)|>Enum.sum
                service_charge=Enum.map(item,fn x -> Decimal.to_float(x.service_charge) end)|>Enum.sum
               ...
               gross_sales=Enum.map(item,fn x -> Decimal.to_float(x.sub_total) end)|>Enum.sum|>Float.round(2)|>Number.Delimit.number_to_delimited()
               service_charges=Enum.map(item,fn x -> Decimal.to_float(x.service_charge) end)|>Enum.sum|>Float.round(2)|>Number.Delimit.number_to_delimited()
               taxes=Enum.map(item,fn x -> Decimal.to_float(x.gst) end)|>Enum.sum|>Float.round(2)|>Number.Delimit.number_to_delimited()
               discounts=after_disc - sub_total|>Number.Delimit.number_to_delimited()
               nett_sales=sub_total + service_charge + gst - discount|>Number.Delimit.number_to_delimited()
               roundings=Enum.map(item,fn x -> Decimal.to_float(x.rounding) end)|>Enum.sum|>Float.round(2)|>Number.Delimit.number_to_delimited()
               total_sales=nett_sale - rounding|>Float.round(2)|>Number.Delimit.number_to_delimited()
               pax=Enum.map(item,fn x -> Decimal.to_float(x.pax) end)|>Enum.sum|>Number.Delimit.number_to_delimited()
                transaction=Enum.map(item,fn x -> x.transaction end)|>Enum.sum|>Number.Delimit.number_to_delimited()
        ...
        discount=after_disc - sub_total
        d_nett_sales=sub_total + service_charge + gst - discount|>Number.Delimit.number_to_delimited()
        d_taxes=Enum.map(a,fn x -> Decimal.to_float(x.gst) end)|>Enum.sum|>Number.Delimit.number_to_delimited()
        d_pax=Enum.map(a,fn x -> Decimal.to_float(x.pax) end)|>Enum.sum
        d_transaction=Enum.map(a,fn x -> x.transaction end)|>Enum.sum


        order_query=Repo.all(from sm in BoatNoodle.BN.SalesMaster,
     ...
     for data <- group_data do
        ...
        Repo.all(
          from sm in BoatNoodle.BN.SalesMaster,
            ...
            left_join: i in BoatNoodle.BN.ItemSubcat, on: sm.itemid==i.subcatid,
            left_join: b in BoatNoodle.BN.Brand, on: b.id==^brand.id,
            where: b.id==i.brand_id and  s.salesdate >= ^payload["s_date"] and
                s.salesdate <= ^payload["e_date"] and s.brand_id==^payload["brand_id"],
                group_by: [i.subcatid,i.itemname],
            select: %{
              id: i.subcatid,
  ...
  %{name: item.itemname,y: Decimal.to_float(item.value)}
        ...
        Repo.all(
           from(sm in BoatNoodle.BN.SalesMaster,
            ...
            left_join: c in BoatNoodle.BN.ItemCat, on: i.itemcatid==c.itemcatid,
             left_join: b in BoatNoodle.BN.Brand, on: b.id==^brand.id,      
            where: c.category_type != "COMBO" and c.itemcatcode != "empty" and b.id==c.brand_id  and s.salesdate >= ^payload["s_date"] and
                s.salesdate <= ^payload["e_date"] and s.brand_id==^payload["brand_id"],
                group_by: [c.itemcatname,s.salesdate],
            select: %{
        ...
        Repo.all(
          from(sm in BoatNoodle.BN.SalesMaster,
            ...
            left_join: c in BoatNoodle.BN.ItemCat, on: i.menu_cat_id==c.itemcatid, 
            left_join: b in BoatNoodle.BN.Brand, on: b.id==^brand.id,     
            where:  b.id==c.brand_id and  s.salesdate >= ^payload["s_date"] and
                s.salesdate <= ^payload["e_date"] and s.brand_id==^payload["brand_id"],
                  group_by: [c.itemcatname,s.salesdate],
            select: %{
