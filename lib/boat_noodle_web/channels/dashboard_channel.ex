defmodule BoatNoodleWeb.DashboardChannel do
  ...
  def handle_in("dashboard", payload, socket) do
    ...
    branchname =
      Enum.map(branch_daily_sales_sumary, fn x -> x.branchname end) |> Enum.uniq() |> hd

    gross_sales =
      Enum.map(branch_daily_sales_sumary, fn x -> x.sub_total end) |> Enum.sum() |> Float.round(2)
      |> Number.Delimit.number_to_delimited()

    service_charge =
      Enum.map(branch_daily_sales_sumary, fn x -> x.service_charge end)
      |> Enum.sum()
      |> Float.round(2)
      |> Number.Delimit.number_to_delimited()

    taxes =
      Enum.map(branch_daily_sales_sumary, fn x -> x.gst end) |> Enum.sum() |> Float.round(2)
      |> Number.Delimit.number_to_delimited()

    discount =
      Enum.map(branch_daily_sales_sumary, fn x -> x.discount end) |> Enum.sum() |> Float.round(2)
      |> Number.Delimit.number_to_delimited()

    nett_sales =
      Enum.map(branch_daily_sales_sumary, fn x -> x.nett_sales end)
      |> Enum.sum()
      |> Float.round(2)
      |> Number.Delimit.number_to_delimited()

    roundings =
      Enum.map(branch_daily_sales_sumary, fn x -> x.roundings end) |> Enum.sum() |> Float.round(2)
      |> Number.Delimit.number_to_delimited()

    total_sales =
      Enum.map(branch_daily_sales_sumary, fn x -> x.total_sales end) |> Enum.sum()
      |> Float.round(2)
    ...
    broadcast(socket, "dashboard", %{
      ...
      pax: pax,
      transaction: transaction,
      branch_daily_sales_sumary: Poison.encode!(branch_daily_sales_sumary),
