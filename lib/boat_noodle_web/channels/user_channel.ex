defmodule BoatNoodleWeb.UserChannel do
  ...
  def handle_in("dashboard", payload, socket) do
    branchid = payload["branch_id"]
       ...
       {total_order,total_transaction}=if branchid == "0" do
      total_order =
        Repo.all(
          from(
  ...
  def handle_in("sales_transaction", payload, socket) do
   ...
   {sales_data,total_pages}=if branchid != "0" do
     ...
     test =if branchid != "0"  do
          ...
          Repo.all(
            from(
              ...
              on: s.salesid == sp.salesid,
              where:
                s.branchid == ^branchid and s.salesdate == ^date and
                  s.brand_id == ^brand_id,
  ...
  def handle_in("item_sold", payload, socket) do
    ...
    item_sold_data =if branchid != "0" do
  ...
  def handle_in("discount_receipt", payload, socket) do
    ...
    brand = Repo.get_by(Brand, id: brand_id)
     ...
     discount_receipt_data =if branchid != "0" do
  ...
  def handle_in("discount_summary", payload, socket) do
      ...
      discount_summary_data =if payload["branch_id"] != "0" do
  ...
  def handle_in("voided_receipt", payload, socket) do
    ...
    brand = Repo.get_by(Brand, id: brand_id)
       ...
       voided_receipt_data =if branchid != "0" do
  ...
  def handle_in("voided_order", payload, socket) do
    ...
    {:ok, end_n} =
     ...
     voided_order_data =if branchid != "0" do
  ...
  def handle_in("sales_summary", payload, socket) do
    ...
    luck =
      for date <- date_data do
        ...
        test =if payload["branch_id"] !="0" do
  ...
  def handle_in("tax", payload, socket) do
    tax_data =if payload["branch_id"] !="0" do
    ...
    tax = if tax_data.tax == nil do
      0.00
    else
      Decimal.to_float(tax_data.tax)
    end

    afterdisc = if tax_data.afterdisc == nil do
      0.00
    else
 ...
 tax_details =if payload["branch_id"] != "0"  do
      ...
      tax_details=for item <- tax_details do

        salesdatetime=DateTime.from_naive!(item.salesdatetime, "Etc/UTC")|>DateTime.to_string|>String.split_at(19)|>elem(0)
  ...
  defp taxes_data(branches, s_date, e_date, brand_id) do
    ...
    for item <- date_data do
      total_transaction =if  branches != "0" do
  ...
  def handle_in("payment_type", payload, socket) do
    {payment_type_cash,payment_type_others,payment} =if payload["branch_id"] != "0" do
      
  
      payment_type_cash=Repo.all(
        from(
          sp in BoatNoodle.BN.SalesPayment,
          left_join: s in BoatNoodle.BN.Sales,
          on: s.salesid == sp.salesid,
    ...
    payment_type_others =
      Repo.all(
        from(
          ...
          where:
           s.is_void == 0 and
            sp.payment_type == "CREDITCARD" and
    ...
    a = if payment_type_others.card == nil do
      "0.00"
    else
      payment_type_others.card
    end

     b = if payment_type_cash.cash == nil do
     "0.00"
  ...
  defp payment_data(branches, s_date, e_date, brand_id) do
    ...
    for item <- date_data do
      total_transaction =if branches != "0"  do
        ...
        Repo.all(
          from(
            ...
            on: sp.salesid == s.salesid,
            where:
              s.is_void == 0and s.salesdate == ^item and
                s.brand_id == ^brand_id,
            select: %{
              salesdate: s.salesdate,
              grand_total: sp.grand_total,
      ...
      {cash,card}=if total_transaction != [] do
        cash =
...
{cash_in,cash_out,cash}=if payload["branch_id"] != "0"  do
    ...
    cash =
      Repo.all(
        from(
          ...
          where:
           c.date_time >= ^new_s_date and
           b.brand_id==^payload["brand_id"] and
              c.date_time <= ^new_e_date and c.brand_id == ^payload["brand_id"],
          select: %{
            branchname: b.branchname,
            amount: sum(c.amount),
    ...
    cash_out = cash_out.cash_out
     ...
     cash_in = if cash_in == nil do
     "0.00"
    else
      cash_in
    end

     cash_out = if cash_out == nil do
     "0.00"
    else
      cash_out
  ...
  def handle_in("update_combo_price", payload, socket) do
    ...
    {p,included_spend}=if a["include_s"] != nil do
      p = a["include_s"] |> hd
    ...
    {p,is_activate}=if a["is_activa"] != nil do
      p = a["is_activa"] |> hd
      is_activate = p.price
       {p,is_activate}
    else
    ...
    {p,is_default_combo}=if a["is_defaul"] != nil do
      p = a["is_defaul"] |> hd
     ...
     {p,enable_disc}=if a["enable_di"] != nil do
      p = a["enable_di"] |> hd
      enable_disc = p.price
        {p,enable_disc}
    else
