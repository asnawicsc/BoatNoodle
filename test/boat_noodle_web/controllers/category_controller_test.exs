defmodule BoatNoodleWeb.CategoryControllerTest do
  use BoatNoodleWeb.ConnCase

  alias BoatNoodle.BN

  @create_attrs %{category_description: "some category_description", category_name: "some category_name", is_default_menu: true}
  ...
  def fixture(:category) do
    {:ok, category} = BN.create_category(@create_attrs)
    category
  end
  ...
  describe "update category" do
    ...
    test "redirects when data is valid", %{conn: conn, category: category} do
      ...
      assert html_response(conn, 200) =~ "some updated category_description"
    end
